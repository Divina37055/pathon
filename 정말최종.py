...
def load_data_from_local():
    """로컬 경로에서 CSV 파일을 자동으로 로드합니다."""
    # Streamlit 앱과 같은 폴더에 있는 CSV 파일
    file_path = "(월)행정구역별 아파트거래량지표.csv"
    ...
    try:
        ...
        if not full_path.exists():
            # 스크립트 파일이 있는 디렉토리 기준으로도 시도
            script_dir = Path(__file__).parent if '__file__' in globals() else current_dir
            full_path = script_dir / file_path

            if not full_path.exists():
                return None, f"파일을 찾을 수 없습니다.\n현재 디렉토리: {current_dir}\n찾는 파일: {file_path}\n\n💡 CSV 파일을 Streamlit 앱(.py 파일)과 같은 폴더에 넣어주세요."

        # 여러 인코딩으로 시도
        for encoding_type in ['cp949', 'utf-8', 'euc-kr']:
            try:
                df = pd.read_csv(full_path, encoding=encoding_type, header=0)
                return df, None
                ...
                continue
        return None, "파일을 로드할 수 없습니다. 지원하는 인코딩(cp949, utf-8, euc-kr)으로 읽을 수 없습니다."
    except Exception as e:
        return None, f"파일을 불러오는 중 오류가 발생했습니다: {str(e)}"
...
def main():
    ...
    data_source = st.sidebar.radio(
        "데이터 불러오기 방식",
        options=["로컬 파일 자동 로드", "파일 직접 업로드"],
        index=0,
        help="기본적으로 로컬 경로의 데이터를 사용합니다."
    )
    ...
    if data_source == "로컬 파일 자동 로드":
        with st.spinner("로컬 파일에서 데이터를 불러오는 중..."):
            df, error = load_data_from_local()
            if error:
                st.error(f"❌ {error}")
                st.info("💡 해결 방법:\n1. 파일 경로가 정확한지 확인\n2. 파일이 해당 경로에 존재하는지 확인\n3. 파일 직접 업로드 방식을 사용")
                ...
                with st.expander("🔍 디버깅 정보"):
                    ...
                    current_dir = Path.cwd()
                    st.code(f"""
찾는 파일명: (월)행정구역별 아파트거래량지표.csv
현재 작업 디렉토리: {current_dir}

설치 방법:
1. Streamlit 앱 파일(.py)이 있는 폴더로 이동
2. 같은 폴더에 CSV 파일을 복사
3. 앱 다시 실행

또는

파일 위치: C:\\Users\\mcp-demo\\data\\project\\
앱 위치: (Streamlit 앱이 실행되는 폴더)

→ CSV 파일을 앱과 같은 폴더로 이동하세요!
                    """)
                return
            else:
                st.success("✅ 로컬 파일에서 데이터를 성공적으로 불러왔습니다!")
                # 데이터 미리보기
                with st.expander("📋 불러온 데이터 미리보기"):
                    st.write(f"총 {len(df)}개 행, {len(df.columns)}개 컬럼")

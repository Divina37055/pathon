...
def load_data_from_github():
    ...
    # GitHub raw URL - 실제 파일명: (월)행정구역별 아파트거래량지표.csv (공백 없음)
    ...
    try:
        ...
        error_msg = str(e)
        if "404" in error_msg:
            return None, f"GitHub에서 파일을 찾을 수 없습니다. 파일 경로를 확인해주세요.\n경로: Divina37055/pathon/main/(월)행정구역별 아파트거래량지표.csv"
        elif "403" in error_msg:
            return None, "GitHub 접근 권한이 없습니다. 저장소가 private인지 확인해주세요."
        else:
            return None, f"GitHub에서 파일을 불러오는 중 오류가 발생했습니다: {error_msg}"
        ...
        with st.spinner("GitHub에서 데이터를 불러오는 중..."):
            ...
            if error:
                ...
                with st.expander("🔍 디버깅 정보"):
                    st.code("""
예상 GitHub URL:
https://raw.githubusercontent.com/Divina37055/pathon/main/(월)행정구역별 아파트거래량지표.csv

확인사항:
1. 저장소명: Divina37055/pathon
2. 브랜치: main
3. 파일명: (월)행정구역별 아파트거래량지표.csv
                    """)
                return
            else:

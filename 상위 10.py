...
def run_analysis(df, input_year, input_month):
    ...
    if input_year < 2020 or input_year > 2025:
        return "[오류] 분석 가능한 기간은 2020년 1월부터 2025년 8월까지입니다.", None
    if input_year == 2020 and input_month < 1:
        return "[오류] 2020년은 1월부터 분석 가능합니다.", None
    if input_year == 2025 and input_month > 8:
        return "[오류] 2025년은 8월까지만 분석 가능합니다.", None
    ...
    if REGION_COL_KEY not in df.columns:
        return f"[오류] 파일에 필수 컬럼 ('{REGION_COL_KEY}')이 없습니다.", None

    # 필요한 컬럼만 선택
    required_cols = [REGION_COL_KEY] + trade_cols_underscore
    missing_cols = [col for col in required_cols if col not in df.columns]
    if missing_cols:
        return f"[오류] 분석에 필요한 월별 데이터가 부족합니다. 파일에 {missing_cols} 컬럼이 없습니다. (현재 파일 컬럼: {list(df.columns)})", None
    ...
    if df_analysis[trade_months].isnull().all().all():
        return "[오류] 선택된 기간의 거래량 데이터가 모두 비어있거나 숫자 형식으로 변환할 수 없습니다.", None
    ...
    if len(df_results_filtered) < 1:
        return "[오류] 필터링 조건을 만족하는 지역이 없습니다. (최근 3개월 거래량이 모두 0보다 크고, 입력월 거래량이 상위 30%에 해당하는 지역)", None
    ...
    # 거래건수 컬럼은 '건' 형식으로 (천 단위 구분 쉼표 추가)
    trade_count_cols = [f'{month} 거래건수' for month in recent_3_months]
    for col in trade_count_cols:
        top_10_results[col] = top_10_results[col].map(lambda x: '{:,.0f}건'.format(x) if x != 0 else '-')

    # 컬럼 순서 재정렬: 지역명, 증가율들, 최근 3개월 거래건수들
    final_cols = ['지역명'] + pct_change_cols + trade_count_cols
    top_10_results = top_10_results[final_cols]

    return None, top_10_results
...
def main():
    ...
    if error_message:
        ...
        **📌 필터링 조건:**
        ...
        st.caption(f"선정 기준: {top_10_results_df.columns[3]} (가장 최근 월의 전월 대비 증가율)")
        st.markdown("---")
